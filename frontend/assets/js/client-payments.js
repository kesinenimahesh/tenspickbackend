...
(function (window, document) {
  ...
  async function loadClients() {
    ...
    try {
      ...
      state.clients = collectionFrom(response, ["clients", "items"]);

      populateClientFilters();

      populateAddClients();
    } catch (err) {
      error("LOAD CLIENTS:", err);
    } finally {
  ...
  async function loadProjectsForFilter() {
    ...
    try {
      ...
      state.projects = collectionFrom(response, ["projects", "items"]);

      populateProjectFilter();
    } catch (err) {
      error("LOAD PROJECTS:", err);
    } finally {
  ...
  async function loadProjectsForClient(clientId) {
    ...
    if (help) {
      help.textContent = "Loading projects for selected client...";
    }
    ...
    try {
      ...
      projects.forEach(function (project) {
        ...
        if (existingIndex >= 0) {
          state.projects[existingIndex] = Object.assign(
            {},
      ...
      if (!projects.length) {
        ...
        if (help) {
          help.textContent = "This client has no available projects.";
        }
      ...
      if (help) {
        help.textContent = "Select the project receiving this payment.";
      }

      return projects;
    } catch (err) {
      error("LOAD CLIENT PROJECTS:", err);
      ...
      if (help) {
        help.textContent = err.message || "Unable to load client projects.";
      }
